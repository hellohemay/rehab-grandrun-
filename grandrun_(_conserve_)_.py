...
def _token_min_value(tok: str | None) -> float:
    """把 '3-4'、'4+'、'NT'、'NA' 轉成可以比較的數值；越小代表越弱。
       - '3-4' 取 3
       - '4+' / '4-' 取 4（只看本體）
       - 'NT' / 'NA' 視為無法比較，回傳 +inf
    """
    ...
    return float(min(int(n) for n in nums)) if nums else float('inf')
...
if __name__ == '__main__':
    # 自行把 diagnosis/pe/html 三段貼上測試不同版本（SCI/TBI/CVA）
    #diagnosis = """(# SCI (spinal cord injury) with T3-4, T4-5 spinal stenosis with associated myelopathy at T3-4, ASIA(American Spinal Injury Association): D, level: L2,
#complicated with paraplegia, neurogenic bladder (foley have removed) and bowel, improving
 #- post T3,T4,T5 total laminectomy on 2025/03/07)"""  # ← CVA 範例（會顯示 Brunnstrom）
    #pe = """([)"""
    # html = """()"""
    # print(generate_report(diagnosis, pe, html))
